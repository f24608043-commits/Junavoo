...
export default function Checkout() {
  ...
  const { formatPrice, convertPrice, t, currency } = useLocale();
  ...
  const subtotalUSD = cartProducts.reduce((sum, p) => sum + p.price * p.qty, 0);
            ...
            <div className="rounded-2xl border bg-card p-6 space-y-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input required value={form.firstName} onChange={handleChange("firstName")} className="mt-1 rounded-xl" />
                </div>
                <div>
                  <Label className="text-xs text-muted-foreground">{t("lastName")}</Label>
                  <Input required value={form.lastName} onChange={handleChange("lastName")} className="mt-1 rounded-xl" />
                </div>
                <div className="sm:col-span-2">
                  <Label className="text-xs text-muted-foreground">{t("email")}</Label>
                  <Input type="email" required value={form.email} onChange={handleChange("email")} className="mt-1 rounded-xl" />
                </div>
                <div className="sm:col-span-2">
                  <Label className="text-xs text-muted-foreground">{t("phone")}</Label>
                  <Input required value={form.phone} onChange={handleChange("phone")} className="mt-1 rounded-xl" />
                </div>
                <div className="sm:col-span-2">
                  <Label className="text-xs text-muted-foreground">{t("address")}</Label>
                  <Input required value={form.address} onChange={handleChange("address")} className="mt-1 rounded-xl" />
                </div>
                <div>
                  <Label className="text-xs text-muted-foreground">{t("city")}</Label>
                  <Input required value={form.city} onChange={handleChange("city")} className="mt-1 rounded-xl" />
                </div>
                <div>
                  <Label className="text-xs text-muted-foreground">{t("zipCode")}</Label>
                  <Input required value={form.zip} onChange={handleChange("zip")} className="mt-1 rounded-xl" />
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border bg-card p-6 space-y-4">
              ...
              <RadioGroup value={payment} onValueChange={setPayment} className="space-y-3">
                <label className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${payment === "cod" ? "border-primary bg-primary/5" : "border-border"}`}>
                  ...
                  <div>
                    <p className="font-medium text-sm">{t("cashOnDelivery")}</p>
          ...
          <div className="rounded-2xl border bg-card p-6 h-fit space-y-4 lg:sticky lg:top-24">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
              {cartProducts.map(p => (
                <div key={p.id} className="flex justify-between text-sm">
                  ...
                  <span className="whitespace-nowrap font-medium">{formatPrice(p.price * p.qty)}</span>
                </div>
              ))}
            </div>
            <div className="space-y-2 text-sm border-t pt-4">
              ...
              <div className="flex justify-between">
                ...
                <span>{shippingUSD === 0 ? t("free") : formatPrice(shippingUSD)}</span>
              </div>
              ...
              {currency === "EUR" && (
                <p className="text-[10px] text-muted-foreground">≈ ${totalUSD.toFixed(2)} USD (1 EUR = $1.19)</p>
              )}
            </div>
