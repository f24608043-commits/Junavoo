...
export default function Shop() {
  ...
  const filtered = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      filteredProducts = filteredProducts.filter(p => p.name.toLowerCase().includes(q) || p.shortDescription.toLowerCase().includes(q));
    }
    ...
    if (minRating > 0) filteredProducts = filteredProducts.filter(p => p.rating >= minRating);
    if (inStockOnly) filteredProducts = filteredProducts.filter(p => p.stock > 0);
    switch (sort) {
  ...
  }, [products, searchQuery, selectedCategories, priceRange, minRating, inStockOnly, sort]);
  ...
  const Sidebar = () => (
    <div className="space-y-7">
      ...
      <div>
        ...
        <Slider min={0} max={100} step={5} value={priceRange} onValueChange={v => { setPriceRange(v); setPage(1); }} className="mt-2" />
