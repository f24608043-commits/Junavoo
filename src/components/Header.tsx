...
import { Search, User, Heart, ShoppingCart, Sun, Moon, Menu, X, Globe, DollarSign } from "lucide-react";
    ...
    >
      <div className="container mx-auto flex h-16 items-center gap-4 px-6 max-w-7xl">
        ...
        <Link to="/" className="text-2xl font-bold shrink-0 tracking-tight">
          <span className="text-foreground">Jun</span><span className="orange-accent">avoo</span>
        </Link>
        ...
        <div className="flex items-center gap-1 shrink-0 ml-auto">
          ...
          <div className="hidden md:flex items-center border rounded-lg overflow-hidden text-xs font-medium">
            <button
              ...
              className={`px-2.5 py-1.5 transition-colors ${language === "en" ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}
            >EN</button>
            <button
              onClick={() => setLanguage("it")}
              className={`px-2.5 py-1.5 transition-colors ${language === "it" ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}
            >IT</button>
          </div>
          ...
          <div className="hidden md:flex items-center border rounded-lg overflow-hidden text-xs font-medium ml-1">
            <button
              ...
              className={`px-2.5 py-1.5 transition-colors ${currency === "USD" ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}
            >$</button>
            <button
              onClick={() => setCurrency("EUR")}
              className={`px-2.5 py-1.5 transition-colors ${currency === "EUR" ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}
            >€</button>
          </div>
          ...
          <Button variant="ghost" size="icon" asChild className="relative rounded-xl">
            <Link to="/wishlist" aria-label={t("wishlist")}>
              ...
              {wishlistCount > 0 && (
                <span className="absolute -top-1 -right-1 h-4 w-4 rounded-full bg-orange-accent text-[10px] font-bold text-white flex items-center justify-center animate-count">
                  {wishlistCount}
                </span>
              )}
            </Link>
              ...
              <div className="bg-popover border rounded-2xl shadow-lg py-2 min-w-[240px] grid grid-cols-2 gap-0.5 p-2">
                {CATEGORIES.map(cat => (
                  <Link
                    ...
                    className="flex items-center gap-2 px-3 py-2 text-sm text-popover-foreground hover:bg-muted rounded-xl transition-colors"
                  >
